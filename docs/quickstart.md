...
```
pyproject.toml → [project.scripts] → tomlrs_cli.cli:main
```

The CLI is invoked as `tomlrs-cli` (or `uvx tomlrs-cli`). Python's entry point mechanism calls `main()` in `src/tomlrs_cli/cli.py`.
...
| Module | Role |
|--------|------|
| `src/tomlrs_cli/__init__.py` | Package marker. No logic. |
| `src/tomlrs_cli/cli.py` | Sole user-facing module. Argument parsing, path resolution, read/write orchestration. |
